...
class CrossQSettings(OffPolicyHyperparamSettings):
    batch_size: int = 128
...
class TorchCrossQOptimizer(TorchOptimizer):
    ...
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        if isinstance(policy.actor, SharedActorCritic):
            raise UnityTrainerException("CrossQ does not support SharedActorCritic")

        hyperparameters: CrossQSettings = cast(
        ...
        self.q_network = TorchCrossQOptimizer.PolicyValueNetwork(
            self.stream_names,
            self.policy.behavior_spec.observation_specs,
            policy_network_settings,
            self._action_spec,
        )
        self.target_q_network = TorchCrossQOptimizer.PolicyValueNetwork(
            self.stream_names,
            self.policy.behavior_spec.observation_specs,
            policy_network_settings,
            self._action_spec,
    ...
    def q_loss(
        ...
        for i, name in enumerate(q_out.keys()):
            ...
            _loss = 0.5 * ModelUtils.masked_mean(
    ...
    def update(self, batch: AgentBuffer, num_sequences: int) -> Dict[str, float]:
        ...
        q2_loss = self.q_loss(q2_out, target_q1, dones, rewards, masks)
