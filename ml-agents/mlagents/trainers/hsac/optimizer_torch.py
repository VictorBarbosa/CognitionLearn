...
class HSACSettings(OffPolicyHyperparamSettings):
    batch_size: int = 128
...
class TorchHSACOptimizer(TorchOptimizer):
    ...
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        if isinstance(policy.actor, SharedActorCritic):
            raise UnityTrainerException("HSAC does not support SharedActorCritic")
        self._critic = ValueNetwork(
            reward_signal_names,
            policy.behavior_spec.observation_specs,
            policy.network_settings,
        )
        hyperparameters: HSACSettings = cast(
        ...
        self.q_network = TorchHSACOptimizer.PolicyValueNetwork(
            self.stream_names,
        ...
        self._log_ent_coef = TorchHSACOptimizer.LogEntCoef(
        ...
        self.target_entropy = TorchHSACOptimizer.TargetEntropy(
    ...
    def update(self, batch: AgentBuffer, num_sequences: int) -> Dict[str, float]:
        ...
        q1_loss, q2_loss = self.hsac_q_loss(
            q1_stream, q2_stream, target_values, dones, rewards, masks
        )
        value_loss = self.hsac_value_loss(
            log_probs, value_estimates, q1p_out, q2p_out, masks
        )
        policy_loss = self.hsac_policy_loss(log_probs, q1p_out, masks)
        entropy_loss = self.hsac_entropy_loss(log_probs, masks)
